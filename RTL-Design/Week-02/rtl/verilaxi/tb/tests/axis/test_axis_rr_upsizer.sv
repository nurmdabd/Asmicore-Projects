...
module test_axis_rr_upsizer #(
  ...
  initial begin
    ...
    $display("[RRU] SRC_BP=%0d SINK_BP=%0d IN=%0d OUT=%0d", src_bp_en, sink_bp_en, IN_DATA_WIDTH,
             OUT_DATA_WIDTH);
  end
  ...
  initial begin
    ...
    forever begin
      ...
      if (m_tvalid_w && m_tready_w) begin
        ...
        logic exp_last;
        $display("[RRU] beat %0d tdata=0x%h tkeep=%03b tlast=%0b", beats_recv, m_tdata_w,
                 m_tkeep_w, m_tlast_w);
        ...
        if (m_tkeep_w !== exp_keep)
          $error(
              "[RRU] FAIL: beat %0d expected tkeep=%03b, got %03b", beats_recv, exp_keep, m_tkeep_w
          );

        if (m_tlast_w !== exp_last)
          $error(
              "[RRU] FAIL: beat %0d expected tlast=%0b, got %0b", beats_recv, exp_last, m_tlast_w
          );

        for (int lane = 0; lane < valid_bytes; lane++) begin
          if (m_tdata_w[lane*8+:8] !== exp_bytes[exp_byte_idx+lane]) begin
            $error("[RRU] FAIL: beat %0d lane %0d expected byte 0x%02h, got 0x%02h", beats_recv,
  ...
  initial begin
    ...
    if (beats_recv !== exp_beats)
      ...
      $error(
          "[RRU] FAIL: consumed %0d expected bytes, expected %0d", exp_byte_idx, exp_bytes.size()
      );
    else
      $display(
          "[RRU] PASS: %0d beats / %0d pkts, byte-accurate scoreboard matched",
          beats_recv,
          pkts_recv
      );
