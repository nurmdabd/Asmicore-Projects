...
OUTPUT_DIR = SCRIPT_DIR.parent / "outputs" / "02_sv_preprocessor"
...
def analyze_file(
    ...
    for include_file in includes:
        ...
        if not include_path.exists():
            result.missing_includes.append(f"{filepath.name}: {include_file}")
    ...
    packages = sorted(set(PACKAGE_IMPORT_RE.findall(text)))
...
def generate_flattened_rtl(
    ...
    output_file = OUTPUT_DIR / "uart_preprocessed.sv"
    ...
    with output_file.open(
        ...
        fp.write("// ============================================================\n")
        fp.write("// PREPROCESSED RTL\n")
        fp.write("// Generated by 02_sv_preprocessor.py\n")
        fp.write("// ============================================================\n\n")
        ...
        for row in entries:
            ...
            source_file = rtl_root / relative_path

            fp.write(
                "// ============================================================\n"
            )

            fp.write(f"// File: {relative_path}\n")
...
def write_dependency_map(
    ...
    report_file = OUTPUT_DIR / "preprocess_dependency_map.md"
    ...
    with report_file.open(
        ...
        fp.write("# Preprocess Dependency Map\n\n")

        fp.write("## Processed Files\n\n")
        ...
        for row in entries:
            ...
            fp.write(f"| {row['relative_path']} | " f"{row['file_category']} |\n")

        fp.write("\n")

        fp.write("## Include Relationships\n\n")

        if result.include_count == 0:

            fp.write("No include directives found.\n\n")
        ...
        fp.write("## Package Dependencies\n\n")

        if result.package_count == 0:

            fp.write("No package dependencies found.\n")
...
def write_issue_log(
    ...
    report_file = OUTPUT_DIR / "preprocess_issues_log.md"
    ...
    with report_file.open(
        ...
        fp.write("# Preprocess Issues Log\n\n")

        fp.write("## Summary\n\n")

        fp.write(f"- Include Directives Found: " f"{result.include_count}\n")

        fp.write(f"- Macro Definitions Found: " f"{result.macro_count}\n")

        fp.write(f"- Conditional Blocks Found: " f"{result.conditional_count}\n")

        fp.write(f"- Package Dependencies Found: " f"{result.package_count}\n\n")

        fp.write("## Parameters\n\n")

        if not result.parameters:

            fp.write("No parameters detected.\n\n")
        ...
        if result.missing_includes:
            ...
            fp.write("### Missing Include Files\n\n")
        ...
        if not issues_found:
            ...
            fp.write("No preprocessing issues detected.\n")
...
def write_pipeline_metadata(
    ...
    metadata["current_stage"] = "sv_preprocessor"

    metadata["generated_by"] = "02_sv_preprocessor.py"

    metadata["preprocessed_files"] = result.processed_files

    metadata["flattened_output"] = "uart_preprocessed.sv"

    metadata["include_count"] = result.include_count

    metadata["macro_count"] = result.macro_count

    metadata["conditional_count"] = result.conditional_count

    metadata["package_count"] = result.package_count
...
def main():
    ...
    metadata_file = manifest_file.parent / "pipeline_metadata.json"
    ...
    rtl_root = Path(metadata["rtl_root"])
    ...
    for row in entries:
        ...
        result.processed_files.append(relative_path)
